...
# bulds a list of the folders that need to be backed up depending on your OS
    ...
    if isdir( "Library" ):
        ...
        return [ "Desktop", "Documents", "Movies", "Music", "Pictures" ]
    else:
        # is Windows
        print( "** Windows user profile detected" )
        return [ "Desktop", "Documents", "Music", "Pictures", "Videos" ]
# end build_dirs
...
# verifies that there is enough space on the destination
    ...
    sfree = popen( "df -h | grep " + mount[:-1] + " | awk '{print $4}'" ).read()[:-1] 
...
def get_help():
    print( '''\nThis script will back up user profile information.
                ...
                Usage:
                    backup.py <src_dir> <mount point> <dest_dir>
...
def main():
    ...
    # argv - 0: backup.py 1: src directory 2: mount point 3: dest directory name
    if 4 != len( argv ): 
        get_help()
        exit()
